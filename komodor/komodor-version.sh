...
# v0.6 use HHMM timestamp prefix for all log output

set -uo pipefail

# ---- Configuration (override via environment) -------------------------------
NAMESPACE="${KOMODOR_NAMESPACE:-komodor}"
KIND="${KOMODOR_KIND:-deployment}"
DEPLOYMENT="${KOMODOR_DEPLOYMENT:-agent-komodor-agent}"
TIMEOUT="${KUBECTL_TIMEOUT:-15s}"
LOG="${LOG_FILE:-$(date +%Y%m%d-%H%M)-komodor-versions.log}"
WORKLOAD_DISPLAY="${DEPLOYMENT} [${KIND}]"
...
Environment overrides:
  ...
  KOMODOR_KIND        Workload kind to inspect     (default: deployment)
  KOMODOR_DEPLOYMENT  Deployment to inspect        (default: agent-komodor-agent)
  KUBECTL_TIMEOUT     Per-request timeout          (default: 15s)
  LOG_FILE            Output log file              (default: <date>-<HHMM>-komodor-versions.log)
EOF
}
...
log " Workload name  : $WORKLOAD_DISPLAY"
...
for ctx in "${CONTEXTS[@]}"; do
  ...
  out=$(kubectl --context "$ctx" --request-timeout="$TIMEOUT" \
    get "$KIND" "$DEPLOYMENT" -n "$NAMESPACE" \
    -o "jsonpath={.metadata.labels['helm\.sh/chart']}" 2>&1)
  rc=$?

  if [[ $rc -ne 0 ]]; then
    if echo "$out" | grep -qiE "not found|notfound"; then
      log "  ➖ NOT-FOUND (Komodor agent not installed as $WORKLOAD_DISPLAY)"
